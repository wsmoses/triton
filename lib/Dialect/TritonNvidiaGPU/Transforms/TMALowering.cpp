...
static void createGlobalToLocalAndWait(ExperimentalDescriptorLoadOp op,
                                       Value dstMem,
  ...
  MemDescType barrierMemDescType = MemDescType::get(
      {1}, IntegerType::get(tensorType.getContext(), 64), barrierEncoding,
  ...
  rewriter.create<triton::nvidia_gpu::AsyncTMACopyGlobalToLocalOp>(
      loc, tmaPtr, op.getIndices(), barrierAlloc, dstMem, pred);
...
public:
  ...
  LogicalResult matchAndRewrite(ExperimentalDescriptorLoadOp op,
    ...
    createGlobalToLocalAndWait(op, alloc, rewriter);
    rewriter.replaceOpWithNewOp<LocalLoadOp>(op, op.getType(), alloc);
    return success();
  }
};

class TMAMMAv2LoadLowering
    : public OpRewritePattern<ExperimentalDescriptorLoadOp> {
public:
  using OpRewritePattern::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(ExperimentalDescriptorLoadOp op,
    ...
    auto order = getOrder(tensorType.getEncoding());
    auto ctaLayout = getCTALayout(tensorType.getEncoding());
    if (tensorType.getRank() == 1 || !op->hasOneUse()) {
      return failure();
    }
    auto localAlloc = dyn_cast<LocalAllocOp>(*op->getUsers().begin());
    if (!localAlloc || !localAlloc->hasOneUse()) {
      return failure();
    }
    auto localLoad = dyn_cast<LocalLoadOp>(*localAlloc->getUsers().begin());
    if (!localLoad || !localLoad->hasOneUse()) {
      return failure();
    }
    auto encoding = localAlloc.getType().getEncoding();
    MemDescType memDescType =
        MemDescType::get(tensorType.getShape(), tensorType.getElementType(),
                         encoding, sharedMemorySpace, /*mutableMemory=*/true);
    Value alloc = rewriter.create<LocalAllocOp>(loc, memDescType);
    createGlobalToLocalAndWait(op, alloc, rewriter);
    localLoad.getSrcMutable().assign(alloc);
    rewriter.eraseOp(localAlloc);
    return success();
  }
};
...
public:
  void runOnOperation() override {
    ...
    patterns.add<TMAMMAv2LoadLowering, TMALoadLowering, TMAStoreLowering,
                 TMACreateDescLowering>(context);
